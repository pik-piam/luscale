...
#' respective shares of the remaining non-cropland vegetation pools within the associated lower resolution spatial
#' units, such as e.g. pasture or primary forest. Urban land is assumed constant over time.  
#' 
...
#'   cropsuit_lr = "avl_land_si.cs3",
#'   cropsuit_hr = "avl_land_si_0.5.mz",
#'   spam = "0.5-to-c200_sum.spam"
#' )
#' }
#'
interpolateCropsuitWeighted <- function(x, x_ini_lr, x_ini_hr, cropsuit_lr, cropsuit_hr, spam, year_ini = "y1985", unit = "Mha") {
  ...
  cropland_diff_lr <- lr[, 2:nyears(lr), "crop"] - setYears(lr[, 1:(nyears(lr) - 1), "crop"], getYears(lr)[2:nyears(lr)])
  # land reduction
  cropland_reduc_lr <- cropland_diff_lr
  cropland_reduc_lr[cropland_reduc_lr > 0] <- 0
  cropland_reduc_lr <- (-cropland_reduc_lr)
  # land expansion
  cropland_expan_lr <- cropland_diff_lr
  cropland_expan_lr[cropland_expan_lr < 0] <- 0

  # calculate shares of non crop land pools in each time step
  non_crop_pools <- getNames(x_ini_lr)[-which(getNames(x_ini_lr) == "crop")]
  non_crop_veg <- non_crop_pools[-which(non_crop_pools == "urban")]
  # calculate shares of non-cropland vegetation pools in terms of total non-cropland vegetation each time step
  shr_non_crop_veg_lr <- x[, , non_crop_veg] / dimSums(x[, , non_crop_veg], dim = 3)
  ...
  if (is.character(cropsuit_hr)) {
    ...
    cropland_suit_hr <- pmin(cropland_suit_hr[,,"si0"], land_non_urban_hr)
    # low resolution
    land_non_urban_lr <- (dimSums(x_ini_lr, dim = 3) - x_ini_lr[, , "urban"])
    cropland_suit_lr <- pmin(cropland_suit_lr[, , "si0"], land_non_urban_lr)
  }

  # compute available cropland for each time step
  cropland_avail_lr <- cropland_suit_lr[, , "si0"] - lr[, , "crop"]
  ...
  cropland_reduc_lr_dagg <- speed_aggregate(cropland_reduc_lr, t(rel))
  cropland_expan_lr_dagg <- speed_aggregate(cropland_expan_lr, t(rel))
  # shares of non-crop land pools
  shr_non_crop_veg_lr_dagg <- speed_aggregate(shr_non_crop_veg_lr, t(rel))
  ...
  cropland_lr_dagg <- speed_aggregate(lr[, , "crop"], t(rel))
  ...
  cropland_hr <- new.magpie(getCells(cropland_expan_lr_dagg), c(year_ini, getYears(cropland_expan_lr_dagg)), "crop")
  getCells(x_ini_hr) <- getCells(cropland_expan_lr_dagg)
  ...
  for (t in 2:nyears(cropland_hr)) {
    ...
    # cropland expansion
    cropland_expan <- cropland_expan_lr_dagg[, t - 1, ]
    # cropland reduction
    cropland_reduc <- cropland_reduc_lr_dagg[, t - 1, ]
    # available cropland in each grid cell (high resolution)
    cropland_avail <- setNames(cropland_suit_hr[, , "si0"] - cropland_hr[, t - 1, ], "crop")
    ...
    cropland_reduc_weight <- cropland_hr[, t - 1, ] / cropland_lr_dagg[, t - 1, ]
  ...
  land_tot_non_crop_veg_hr <- setNames(setYears(land_tot_hr - x_ini_hr[, , "urban"] - cropland_hr[, 2:nyears(cropland_hr), ], getYears(cropland_hr)[2:nyears(cropland_hr)]), NULL)

  # create new magpie object
  land_non_crop_veg_hr <- new.magpie(getCells(cropland_hr), getYears(cropland_hr), getNames(shr_non_crop_veg_lr_dagg))
  land_non_crop_veg_hr[, year_ini, non_crop_veg] <- x_ini_hr[, , non_crop_veg]

  # disaggregate non-cropland for each time step
  land_non_crop_veg_hr[, 2:nyears(land_non_crop_veg_hr), non_crop_veg] <- land_tot_non_crop_veg_hr * shr_non_crop_veg_lr_dagg

  # calculate disaggregated urban land pool
  land_urban_hr <- setYears(setNames(land_tot_hr - (cropland_hr + dimSums(land_non_crop_veg_hr, dim = 3)), "urban"), getYears(cropland_hr))

  # combine output
  hr <- mbind(cropland_hr, land_non_crop_veg_hr, land_urban_hr)
