...
#' @importFrom magclass is.magpie nregions nyears getNames getYears<- getCells<- mbind dimSums setYears getYears as.magpie new.magpie as.array
#' @importFrom madrat toolAggregate
#' @seealso \code{\link[madrat]{toolAggregate}}
#' @export
interpolate2 <- function(x, x_ini, map, x_ini_lr=NULL){
  if(!is.magpie(x) || !is.magpie(x_ini)) stop("x and x_ini have to be magpie objects")
  if(is.null(getYears(x))) stop("Years must be defined in x.")
  if(nyears(x_ini)>1) stop("Initialization data must only contain one timestep")
  if(!setequal(getNames(x),getNames(x_ini))) stop("data names of x and x_ini must be identical")
  
  x <- x[,sort(getYears(x,as.integer=TRUE)),]
  if(is.null(getYears(x_ini))) {
    getYears(x_ini) <- getYears(x,as.integer=TRUE)[1] -1
  } else if(getYears(x_ini, as.integer=TRUE) >=  getYears(x,as.integer=TRUE)[1]) {
  ...
  hr <- new.magpie(getCells(x_ini),c(getYears(x_ini),getYears(reduct_hr)),getNames(x_ini))
  hr[,1,] <- x_ini
  getCells(reduct_hr) <- getCells(extent_hr) <- getCells(x_ini)

  for(y in 1:nyears(reduct_hr)) {
    hr[,y+1,] <- (1-reduct_hr[,y,]) * setYears(hr[,y,],NULL) + 
                 dimSums(reduct_hr[,y,] * setYears(hr[,y,],NULL), dim=3) * extent_hr[,y,]
  }
  return(hr)
}